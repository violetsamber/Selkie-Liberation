/*
    File: fn_taskNotificationHandler.sqf
    Authors: Violets
    Date: 2025-11-20
...
switch (_taskType) do {
    case "LIB_Defend": {
        ...
        if(_state == "CREATED" && SLKLIB_notification_friendly_sector_attacked) then {
            [ "lib_sector_attacked", [ markerText _sector ] ] call BIS_fnc_showNotification;
        };
        if(_state == "FAILED" && SLKLIB_notification_friendly_sector_lost) then {
            [ "lib_sector_lost", [ markerText _sector ] ] call BIS_fnc_showNotification;
        };
        if(_state == "SUCCEEDED" && SLKLIB_notification_friendly_sector_lost) then {
