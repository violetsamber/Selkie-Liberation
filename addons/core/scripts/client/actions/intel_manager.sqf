    ...
    if ([5] call KPLIB_fnc_hasPermission) then {
...

        {
            if !(_x in _actionned_intel_items) then {
                _x addAction ["<t color='#FFFF00'>" + localize "STR_INTEL" + "</t>",{[_this select 0] remoteExecCall ["KPLIB_shared_fnc_intel_remote_call", 2];},"",-849,true,true,"","(vehicle player == player)"];
