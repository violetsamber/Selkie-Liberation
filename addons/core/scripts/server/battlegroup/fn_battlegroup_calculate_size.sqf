...
if(_timeSince < _minSleep) then {
    ADD(_size,-1);
    [format ["Below min time penalty: Size: %1",_size], "BATTLEGROUP"] call KPLIB_fnc_log;
};
...
if(_timeSince > _minSleep && _timeSince < _sleepFirstThird) then {
    //Do Nothing
    [format ["Min Sleep: Size: %1",_size], "BATTLEGROUP"] call KPLIB_fnc_log;
};

//Center 1
if(_timeSince >= _sleepFirstThird && _timeSince < _sleepSecondThird) then {
    ADD(_size,1);
    [format ["Center Sleep: Size: %1",_size], "BATTLEGROUP"] call KPLIB_fnc_log;
};

//Max 2
if(_timeSince >= _sleepSecondThird) then {
    ADD(_size,2);
    [format ["Max Sleep: Size: %1",_size], "BATTLEGROUP"] call KPLIB_fnc_log;
};

//Combat Readiness 0-100 [0.5x, 1x, 2x]
private _readiness = SLKLIB_combat_readiness;
private _readinessThirds = ((100 - 0) / 3);
private _readinessFirstThird = (_minSleep + _sleepThirds);
private _readinessSecondThird = (_minSleep + (_sleepThirds * 2));

if(_readiness < _readinessFirstThird) then {
    _size = (_size * 0.5);
    [format ["Readiness Min: Size: %1",_size], "BATTLEGROUP"] call KPLIB_fnc_log;
};

if(_readiness >= _readinessFirstThird && _readiness < _readinessSecondThird) then {
    _size = (_size * 1);
    [format ["Readiness Center: Size: %1",_size], "BATTLEGROUP"] call KPLIB_fnc_log;
};

if(_readiness >= _readinessSecondThird) then {
    _size = (_size * 2);
    [format ["Readiness Max: Size: %1",_size], "BATTLEGROUP"] call KPLIB_fnc_log;
};
...
switch (_sectorType) do {
    case SECTOR_TYPE_TOWER: {
        ADD(_size,-1);
        [format ["Type %1: Size: %2", SECTOR_TYPE_TOWER, _size], "BATTLEGROUP"] call KPLIB_fnc_log;
    };
    case SECTOR_TYPE_TOWN: {
        //ADD(_size,0);
        [format ["Type %1: Size: %2", SECTOR_TYPE_TOWN, _size], "BATTLEGROUP"] call KPLIB_fnc_log;
    };
    case SECTOR_TYPE_FACTORY: {
        ADD(_size,1);
        [format ["Type %1: Size: %2", SECTOR_TYPE_FACTORY, _size], "BATTLEGROUP"] call KPLIB_fnc_log;
    };
    case SECTOR_TYPE_MILITARY: {
        ADD(_size,2);
        [format ["Type %1: Size: %2", SECTOR_TYPE_MILITARY, _size], "BATTLEGROUP"] call KPLIB_fnc_log;
    };
    case SECTOR_TYPE_BIGTOWN: {
        ADD(_size,2);
        [format ["Type %1: Size: %2", SECTOR_TYPE_BIGTOWN, _size], "BATTLEGROUP"] call KPLIB_fnc_log;
    };
};
