...
private _planes_number = ((floor linearConversion [40, 100, SLKLIB_combat_readiness, 1, 3]) min 3) max 0;
...
for "_i" from 1 to _planes_number do {
    ...
    _plane addMPEventHandler ["MPKilled", {_this spawn KPLIB_shared_fnc_kill_manager}];
    [_plane] call KPLIB_fnc_addObjectInit;
    {_x addMPEventHandler ["MPKilled", {_this spawn KPLIB_shared_fnc_kill_manager}];} forEach (crew _plane);
