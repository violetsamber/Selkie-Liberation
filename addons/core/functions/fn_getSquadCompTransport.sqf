/*
    File: fn_getSquadCompTransport.sqf
...
if (_type == "army") then {
    ...
    if (air_weight > 40 && !_selected) then {
        ...
        if ((random 100) < _randomchance) then {
            _selected = true;
            _squadcomp = KPLIB_o_squadAir_Transport;
        };
    };
    ...
    if (infantry_weight > 40 && !_selected) then {
        ...
        if ((random 100) < _randomchance) then {
            _selected = true;
            _squadcomp = KPLIB_o_squadInf_Transport;
        };
    };
} else {
