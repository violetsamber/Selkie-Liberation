...
while {dialog && alive player && (dobuild == 0 || buildtype == 1)} do {
        ...
        lbClear 110;
        {
            ...
            if (buildtype != 9) then {
                ...
                switch (_classnamevar) do {
                    ...
                    case KP_liberation_spartan_building: {_entrytext = localize "STR_SPARTAN_BUILDING";};
                    case KP_liberation_ODST_building: {_entrytext = localize "STR_ODST_BUILDING";};
    ...
    _affordable = false;
    _squad_full = false;
    if ((buildtype == 1 || buildtype == 8) && (count (units group player) >= GRLIB_max_squad_size)) then {
        _squad_full = true;
    };
    _linked = false;
    ...
    if (dobuild == 0 && _selected_item != -1 && (_selected_item < (count _build_list))) then {
        ...
        if (
            ...
            if !((_build_item select 0) isEqualType []) then {
                if ((toLower (_build_item select 0)) in KPLIB_b_spartan_classes) then {
                    if (KP_liberation_spartan_building_near) then {
                        _affordable = true;
                    }
                } else {
    ...
    if ( unitcap >= ([] call KPLIB_fnc_getLocalCap)) then {
        ...
        if (buildtype == 1 || buildtype == 8 || buildtype == 9) then {
            _affordable = false;
        };
    };
    ...
    if((buildtype == 1 || buildtype == 8) && dobuild != 0) then {
