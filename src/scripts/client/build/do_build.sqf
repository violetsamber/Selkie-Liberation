...
while { true } do {
    ...
    if(buildtype == 1 || buildtype == 8) then {
        ...
        _classname createUnit [_pos, _grp,"this addMPEventHandler [""MPKilled"", {_this spawn KPLIB_shared_fnc_kill_manager}]", 0.5, "private"];
        build_confirmed = 0;
    } else {
        if ( buildtype == 9 ) then {
            _pos = [(getPos player select 0) + 1,(getPos player select 1) + 1, 0];
            ...
            _idx = 0;
            {
                ...
                if (_classname isEqualTo blufor_squad_para) then {
                    _x createUnit [_pos, _grp,"this addMPEventHandler [""MPKilled"", {_this spawn KPLIB_shared_fnc_kill_manager}]; removeBackpackGlobal this; this addBackpackGlobal ""B_parachute""", 0.5, _unitrank];
                } else {
                    _x createUnit [_pos, _grp,"this addMPEventHandler [""MPKilled"", {_this spawn KPLIB_shared_fnc_kill_manager}];", 0.5, _unitrank];
                };
                _idx = _idx + 1;
