...
if (KP_liberation_asymmetric_debug > 0) then {[format ["Sector %1 (%2) - KPLIB_server_fnc_sector_guerilla spawned on: %3", (markerText _sector), _sector, debug_source], "ASYMMETRIC"] remoteExecCall ["KPLIB_fnc_log", 2];};
...
if (((random 100) <= 25) && (KP_liberation_guerilla_vehicles isNotEqualTo [])) then {
...
if (KP_liberation_asymmetric_debug > 0) then {[format ["Sector %1 (%2) - KPLIB_server_fnc_sector_guerilla dropped on: %3", (markerText _sector), _sector, debug_source], "ASYMMETRIC"] remoteExecCall ["KPLIB_fnc_log", 2];};
